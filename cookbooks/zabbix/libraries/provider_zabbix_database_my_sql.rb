...
class Chef
  class Provider
    class ZabbixDatabase
      class 
        MySql < Chef::Provider::ZabbixDatabase
        ...
        def create_new_database
          ...
          zabbix_path = case node['zabbix']['server']['install_method']
            when 'package' then
              ::File.join(new_resource.source_dir, "zabbix-server-mysql-#{new_resource.server_version}")
            else
              ::File.join(new_resource.source_dir, "zabbix-#{new_resource.server_version}")
            end
          sql_scripts = if new_resource.server_version.to_f < 2.0
            ...
            [
              ...
              when 'package' then
                [
                  ["zabbix_populate_schema", ::File.join(zabbix_path, 'create', "schema.sql")],
                  ["zabbix_populate_data",   ::File.join(zabbix_path, 'create', "data.sql")],
                  ["zabbix_populate_image",  ::File.join(zabbix_path, 'create', "images.sql")],
                ]
              else
